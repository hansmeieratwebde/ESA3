...
function show_files(id) {
    ...
    for (f in files) {
        ...
        /*get thumbnail according to file type */
        show_thumbnails(id, ext, files[f]);
...
/*show thumbnails according to filetype */
function show_thumbnails(id, ext, file) {
    if (ext == "jpg") {
        /* Check if we are in "thumbnails" folder */
        if (get_path(id).indexOf("thumbnails") !== -1) {
            //if we are in thumbnails folder show file as thumbnail
            var thumbnail_url = path_to_upload_folder + get_path(id).substr(1) + file;

        }
        else {
            //otherwise show thumbnail file in thumbnail folder
            var thumbnail_url = path_to_upload_folder + get_path(id).substr(1) + "thumbnails/" + file + thumbnail_extension;
        }
        appendImgToDom(thumbnail_url);
    }
    else {

        //if file is not jpeg show icon for file type
        var thumbnail_url = static_url + "filemanager/images/icons/" + ext + ".png";
        ...
        $.get(thumbnail_url)
            ...
            .fail(function () {
                //show default icon if no icon file
                var default_url = static_url + "filemanager/images/icons/default.png";
                appendImgToDom(default_url);
            })
    }
